...
def get_spotipy_data(driver,myquery,limit):
  ...
  for idx, track in enumerate(results['tracks']['items']):
    ...
    yt_views=int(getViews(driver,track_name).replace(',',''))
    print(yt_views)
...
def search(request):
    ...
    if request.GET['keyword']!="":
      if request.GET['type']!="song":
        mydata,info_stat=get_spotipy_data(driver,request.GET['type']+':'+request.GET['keyword'],50)
      else:
        mydata,info_stat=get_spotipy_data(driver,request.GET['keyword'],50)
    print(mydata)
    print(info_stat)
    driver.quit()
    print("done!")
